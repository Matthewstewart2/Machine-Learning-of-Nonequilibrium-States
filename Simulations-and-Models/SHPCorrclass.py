...
class Translation(object):
    ...
    def next_move(self, writer, sample_diff):
        ...
        if self.hit_ss == True and len(self.written_pts) < 200 and (self.total_t + delta_t) > (self.t0 + self.sample_time):
            self.written_pts.append(self.get_true_occs_array()[round(self.true_length / 2)])
            self.sample_time += sample_diff
            if len(self.written_pts) == 200 and self.written == False:
                writer.writerow(self.written_pts)
                print('csv written')
                self.written = True
    ...
    def play(self, writer, sample_diff):
        # play the simulation
        segment = 1000
        num_parts1 = []
        num_parts2 = []
            ...
            for i in range(self.sweeps * self.true_length):
                if not self.hit_ss:
                    ...
                    if len(num_parts1) == segment and len(num_parts2) == segment:
                        ...
                        num_parts1 = []
                        num_parts2 = []
                self.next_move(writer, sample_diff)
            self.loops += 1
