...
class Translation(object):
    ...
    def __init__(self, alpha, beta, rates_list, true_length, part_size, sweeps, gamma):
        ...
        self.ss_frames = 0
        self.rows_written = 0
        self.max_rows = 2 * 10**4
        self.sample_time = 0
        self.sample_diff = 9.0
    ...
    def next_move(self, writer):
        ...
        if self.hit_ss == True and self.rows_written < self.max_rows and (self.total_t + delta_t) > (self.t0 + self.sample_time):
            writer.writerow(list(self.get_true_occs_array()))
            self.rows_written += 1        
            self.sample_time += self.sample_diff
            if self.rows_written == self.max_rows:
                print('{}, {} csv done, rows written: {}'.format(self.alpha, self.beta, self.rows_written))
    ...
    def play(self):
        # play the simulation
        broken = False
        with open('CXpowelawnegative1_5{}alpha{}beta{}tdiff{}.csv'.format(self.max_rows, self.alpha, self.beta, self.sample_diff), 'w', encoding='UTF8', newline='') as f:
            writer = csv.writer(f)
            segment = 1000
            num_parts1 = []
            num_parts2 = []
